...
document.getElementById('feesForm').addEventListener('submit', async (e) => {
    ...
    if (course === "select" || !uid) {
        alert("Please select a course and enter your USN number.");
        return;
    }
...
document.getElementById('feesForm').addEventListener('submit', function (e) {
    ...
    if (course === 'select' || !uid) {
      alert('Please select a course and enter USN number');
      return;
    }
    ...
    .then(data => {
      if (data.success) {
        // Populate student name and fees
        document.getElementById('name').value = data.name;
        document.getElementById('fees').value = data.fees;
      } else {
        alert(data.message);
